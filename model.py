...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    if augment:
        ...
        if random_num == 1:
            image, mask, class_ids = augment_images.make_new_image(image,
                                                                   mask,
                                                                   width = config.IMAGE_MAX_DIM,
                                                                   height = config.IMAGE_MAX_DIM,
                                                                   show = False, 
                                                                   object_num = random.randint(25,75),
                                                                   class_ids = class_ids,
                                                                   method = random.randint(1,2) )
        elif random_num == 2:
            image = augment_images.apply_image_augmentation(image,aug = None)
            #image = np.fliplr(image)
            #mask = np.fliplr(mask)
        elif random_num == 3:
            image,mask = augment_images.apply_image_and_mask_augmentation(image,mask.astype('uint8'),aug = None,swap_dim=False)
        elif random_num == 4:
            image = augment_images.apply_image_augmentation(image,aug = None)
            image,mask = augment_images.apply_image_and_mask_augmentation(image,mask.astype('uint8'),aug = None,swap_dim=False)
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    while True:
        try:
            ...
            image_index = dataset.get_next_index() #new command to load classes in a balanced manner
...
class MaskRCNN():
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.checkpoint_path = os.path.join(self.log_dir, "mask_rcnn.h5")#_{}_*epoch*.h5".format(
        #    self.config.NAME.lower()))
        #self.checkpoint_path = self.checkpoint_path.replace(
        #    "*epoch*", "{epoch:04d}")

    def evaluate_unclassified(self, dataset,learning_rate,layers,debug=True):
        ...
        dataset_generator = data_generator(dataset,
                                           self.config,
                                           augment=False,
                                           shuffle=False,
                                           batch_size=1)
        ...
        if debug==True:
            for i in range(5):
              x,y = next(dataset_generator)
              scores = np.array(self.keras_model.evaluate(x,y))
              image_info = {dataset.filenames[i]:{
                          'loss':str(scores[0][0][0]),
                          'rpn_class_loss': str(scores[1]),
                          'rpn_bbox_loss':str(scores[2]),
                          'mrcnn_class_loss':str(scores[3]),
                          'mrcnn_bbox_loss':str(scores[4][0][0]),
                          'mrcnn_mask_loss':str(scores[5][0][0]) }}
             
              loss_scores.update(image_info)
            return loss_scores
        
        for i in range(dataset.image_number):
            x,y = next(dataset_generator)
            scores = np.array(self.keras_model.evaluate(x,y))
            image_info = {dataset.filenames[i]:{
                          'loss':str(scores[0][0][0]),
        ...
        #scores = self.keras_model.evaluate_generator(dataset_generator, use_multiprocessing=False,steps=dataset.image_number)
        return loss_scores
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        callbacks = [
            ...
            #keras.callbacks.EarlyStopping(monitor='val_mrcnn_mask_loss',
...
def unmold_image2(normalized_images, config):
    """Takes a image normalized with mold() and returns the original."""
